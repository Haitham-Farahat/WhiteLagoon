...
namespace WhiteLagoon.Application.Common.Utility
{
    public class SD
    {
        ...
        public static RedialBarChartDto GetRadialBarCharDataModel(int totalCount, double currentMonthCount, double prevMonthCount)
        {
            ...
            if (prevMonthCount != 0)
            {
                increaseDecreaseRatio = Convert.ToInt32((currentMonthCount - prevMonthCount) / (double)prevMonthCount * 100);
            }


            redialBarChartDto.TotalCount = totalCount;
            redialBarChartDto.CountInCurrentMonth = Convert.ToInt32(currentMonthCount);
            redialBarChartDto.hasRationIncreased = currentMonthCount > prevMonthCount;
